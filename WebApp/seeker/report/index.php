...
if($rowregister->image == '0'){
	...
	$imagePath = '../../seeker/images/'.$seekerid.'.jpg';
}



$sql = "SELECT p.dateAdd, IFNULL(x.jobnames, 'Any') as jobnames, IFNULL(y.location, CONCAT('All ', l.location)) AS sub_location, l.location location,  
...
if($profile->pstartType == 1){
	$startDate = '<span style="font-size:10px;">Immediate Start</span>';
}else{
	$startDate = $profile->pstartDate;
}
if($profile->pendType== 1){
	$endDate = '<span style="font-size:10px;">Any Date can end</span>';
}else{
...
$stylesheet = file_get_contents('../../seeker/report/index.css');
...
<tr>
<td colspan="2">
I am Looking for '.$profile->jobnames.' job(s) in '.$profile->location.' greater suburbs. I mostly prefer '.$profile->sub_location.' subsidiary areas to undertake the work assigned. 
</td>
</tr>
</tbody>
</table>
