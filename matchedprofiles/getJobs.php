...
$empID = $_SESSION['id'];
$profileID = $_POST['jobID'];

	$sql = "SELECT j.dateAdd, IFNULL(x.jobnames, 'Any') AS jobnames,
	CASE WHEN j.jobType=0 THEN 'ANY' 
	WHEN j.jobType=1 THEN 'Casual' 
	WHEN j.jobType=2 THEN 'Part-time' 
	WHEN j.jobType=3 THEN 'One-Off' 
	END as jobTypeName, l.location as location, IFNULL(y.location, CONCAT('All ', l.location)) AS location_sub, j.payRate, j.startDate, j.startType, j.endDate, j.endType, j.fromTime, j.toTime, j.totalHours, j.timeDiff, 
	...
	WHEN j.ethnicity=5 THEN 'MELAA (Middle Eastern/Latin American/African)' END AS ethnicity, j.age, 
	...
	FROM profile j 
	LEFT JOIN (SELECT pj.profileid, GROUP_CONCAT(jt.jobTitle) as jobnames FROM profilemultijobtitles pj LEFT JOIN jobtitle jt ON pj.jobTitleid = jt.id GROUP BY pj.profileid) x ON x.profileid = j.id 
	LEFT JOIN (SELECT pl.profileid, GROUP_CONCAT(sl.location) as location FROM
	profilemultilocationsub pl LEFT JOIN locations_sub sl ON pl.locationSubid = sl.id GROUP BY pl.profileid) y ON y.profileid = j.id
	LEFT JOIN locations l ON j.location = l.id  
	LEFT JOIN timediffprofiles t ON t.profileid = j.id WHERE j.userid = ? AND j.id = ?";
	$smt = $con->prepare($sql);
	$smt->execute(array($empID, $profileID));
	...
	LEFT JOIN job j ON j.id=jm.jobid 
		LEFT JOIN register r ON j.userid = r.id 
	LEFT JOIN jobtitle jtitle ON j.jobTitleid = jtitle.id 
